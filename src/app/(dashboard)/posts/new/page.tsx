  ...
  const handleSaveDraft = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setFeedback({
          type: "error",
          message: errData?.error?.message ?? "Gagal menyimpan post.",
        });
      }
    } catch {
  ...
  const handlePublishNow = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setFeedback({
          type: "error",
          message: errData?.error?.message ?? "Gagal mempublikasikan post.",
        });
      }
    } catch {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-3 pb-5 border-b border-zinc-800/80">
        ...
        <div>
          <h1 className="text-xl font-bold tracking-tight text-zinc-100">Buat Post Baru</h1>
          <p className="text-xs text-zinc-400 mt-0.5">Tulis konten dan pilih platform tujuan.</p>
        </div>
      </div>
      ...
      {feedback && (
        <div
          className={`flex items-start gap-3 p-4 rounded-xl border text-xs transition-all ${
          ...
          <Card className="border-zinc-800 bg-zinc-900/40 rounded-xl">
            <CardContent className="p-5 space-y-3">
              <div className="flex items-center justify-between">
                ...
                <span className="text-[11px] text-zinc-500">Opsional</span>
              </div>
              ...
              <p className="text-[11px] text-zinc-600">
                Gambar: JPEG, PNG, GIF (maks 8 MB) · Video: MP4, MOV (maks 512 MB)
              </p>
            </CardContent>
          </Card>
            ...
            <CardContent className="p-5 space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-xs font-semibold text-zinc-300">
                  Akun Tujuan <span className="text-red-400">*</span>
                </span>
              ...
              {isLoadingAccounts ? (
                ...
                <div className="py-6 text-center text-xs text-zinc-500 space-y-2">
                  <p className="text-zinc-400">Belum ada akun yang terhubung.</p>
                  <Button
                    variant="outline"
                    size="sm"
                  ...
                  {accounts.map((acc) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div
                          className={`w-4 h-4 rounded-md border-2 shrink-0 flex items-center justify-center transition-colors ${
            ...
            <CardContent className="p-5 space-y-3">
              <div className="flex items-center justify-between">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    ...
                    className="w-3.5 h-3.5 rounded bg-zinc-800 border-zinc-600 text-zinc-100 focus:ring-zinc-500"
                  />
              ...
              {useSchedule && (
                <input
                  ...
                  className="w-full bg-zinc-800 border border-zinc-700 rounded-lg px-4 py-2.5 text-xs text-zinc-200 focus:outline-none focus:border-zinc-500 transition-colors"
                />
              )}
            </CardContent>
          </Card>
