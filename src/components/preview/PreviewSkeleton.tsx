...
export function PlatformPreviewSkeleton({ platform }: { platform?: SupportedPlatform }) {
  return (
    <div
      ...
      className="w-full max-w-md mx-auto bg-slate-900 border border-slate-800 rounded-xl overflow-hidden shadow-sm animate-pulse"
    ...
    >
      ...
      <div className="w-full h-48 sm:h-56 bg-slate-800/50 flex items-center justify-center">
        <div className="w-8 h-8 rounded-lg bg-slate-700/40" />
      </div>

      {/* Action buttons footer skeleton */}
      <div className="p-3 border-t border-slate-800/60 flex items-center justify-around">
...
export function PreviewPanelSkeleton() {
  return (
    <div
      ...
      className="flex flex-col gap-4 rounded-xl border border-slate-800 bg-slate-900/70 p-4 shadow-sm min-h-[460px] animate-pulse"
      aria-busy="true"
    ...
    >
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-800 pb-3">
        ...
        <div className="flex items-center gap-1.5 p-1 bg-slate-950/80 rounded-lg border border-slate-800">
          <div className="w-20 h-7 rounded-md bg-slate-800" />
          <div className="w-20 h-7 rounded-md bg-slate-800/50 hidden sm:block" />
          <div className="w-20 h-7 rounded-md bg-slate-800/50 hidden md:block" />
        </div>
